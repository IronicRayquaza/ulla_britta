 ...
 *
 * These cover the two properties that must never regress:
 *  1. An irreversible tool cannot run without an explicit confirmation from a
 *     later user turn (so the agent cannot confirm its own destructive action).
 *  2. The agent never reports success for work it did not perform.
 *
...
const suites = ['destructive-gate.test.mjs', 'honest-failure.test.mjs'];
